...
:::caution
请务必从盒子网口插好网线连接到路由器的 LAN 口！否则无法正常网口刷机！    
:::
...
- 选择芯片hi3798mv200，选择串口为你的转接器COM口，点击刷新COM口检查是否正确，  
...
4.看教程都需要一（亿）点点天赋的，不会刷，买一个作者刷好的盒子不香吗？  
...
- ttyAMA0的登录账密默认就是系统的后台默认初始账密 ```root``` ```ecoo1234``` （密码盲输！）
